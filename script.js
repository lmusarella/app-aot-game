...
class UnitPopupsView {
  ...
  _renderList(type, ul){ if (!ul) return; const s=this.store.get(); const data = type==='recruit'? s.recruitsData : s.commandersData; ul.replaceChildren();
...
class TitansView {
  ...
  _switchType(id){ this.store.set(prev=>({ ...prev, titansData: prev.titansData.map(t=>{
